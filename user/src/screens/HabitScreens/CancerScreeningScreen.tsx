...
const GROUPS = [
  {
    id: "women-under40",
    titleKey: "cancerScreen.groupWomenUnder40",
    title: "Women, under 40",
    tabKey: "cancerScreen.tabWomenUnder40",
    accent: "#C1682E",
    tests: [
      {
        ...
        freq: "Monthly",
        noteKey: "testNote.breastSelfExam",
        ...
        evidenceKey: "testEvidence.breastSelfExam",
        ...
        video: { labelKey: "testVideo.nhsBreast", label: "NHS — How to check your breasts or chest", url: "https://www.youtube.com/watch?v=xcg7jWrlLJ8" },
      },
      {
        nameKey: "testName.papSmear",
        name: "Pap smear",
        freqKey: "testFreq.papSmearUnder40",
        freq: "Every 3 years, from age 21 (or within 3 years of first intercourse)",
        noteKey: "testNote.papSmear",
        note: "Screens for cervical pre-cancer/cancer.",
      },
      {
        nameKey: "testName.hpvVaccine",
        name: "HPV vaccination",
        freqKey: "testFreq.hpvVaccine",
        freq: "One-time series if not already given",
        noteKey: "testNote.hpvVaccine",
  ...
  {
    id: "women-over40",
    titleKey: "cancerScreen.groupWomenOver40",
  ...
  {
    id: "men-smokers",
    titleKey: "cancerScreen.groupMenSmokers",
    title: "Men who smoke, any age",
    tabKey: "cancerScreen.tabMenSmokers",
    accent: "#5B4A8A",
    tests: [
      {
        nameKey: "testName.ldct",
        name: "Low-dose CT chest (LDCT)",
        freqKey: "testFreq.yearly",
        freq: "Yearly",
        noteKey: "testNote.ldct",
  ...
  },
  {
    ...
    titleKey: "cancerScreen.groupMen60Plus",
    title: "Men, non-smokers, 60 and over",
    tabKey: "cancerScreen.tabMen60Plus",
    accent: "#3E7C6B",
    tests: [
      {
        nameKey: "testName.psa",
        name: "S. PSA",
        freqKey: "testFreq.yearly",
        freq: "Yearly",
        noteKey: "testNote.psa",
        note: "Prostate-specific antigen, for prostate cancer risk awareness.",
        evidenceKey: "testEvidence.psa",
...
const SYMPTOMS = [
  { key: "symptom.mouthUlcer", text: "Non-healing mouth ulcer, or a white/red patch inside the mouth" },
  { key: "symptom.hoarseness", text: "Persistent hoarseness of voice or a cough" },
  { key: "symptom.lump", text: "A lump or thickening anywhere — breast, neck, testicle, or elsewhere" },
  { key: "symptom.bleeding", text: "Unusual bleeding or discharge — between periods, after menopause, after intercourse, or blood in urine/sputum" },
  { key: "symptom.bloodStools", text: "Blood in stools, or black/tarry stools" },
  { key: "symptom.stoolRoutine", text: "Change in stool passing routine — new constipation, diarrhoea, or altered frequency lasting beyond a few days" },
  { key: "symptom.bladder", text: "Change in bladder habits" },
  { key: "symptom.swallowing", text: "Difficulty swallowing, or persistent indigestion/acidity" },
  { key: "symptom.weightLoss", text: "Unexplained weight loss" },
  { key: "symptom.fatigue", text: "Persistent fatigue that doesn't improve with rest" },
  { key: "symptom.skinSore", text: "A sore that doesn't heal, or a mole/wart that changes size, shape, or colour" },
  { key: "symptom.pain", text: "Unexplained, persistent pain — abdominal, bone, or otherwise" },
  { key: "symptom.fever", text: "Unexplained fever or night sweats" },
];
  ...
  return (
    <div className="py-4 border-b border-slate-100 dark:border-slate-800 last:border-0 animate-in fade-in duration-200">
      <div className="flex justify-between items-baseline gap-2">
        <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">{t(test.nameKey, test.name)}</h4>
        <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 shrink-0">{t(test.freqKey, test.freq)}</span>
      </div>
      <p className="text-xs text-slate-600 dark:text-slate-350 mt-1 leading-relaxed">{t(test.noteKey, test.note)}</p>
      ...
      {test.video && (
        ...
        >
          ▶️ {t(test.video.labelKey, test.video.label)}
        </a>
      )}
      ...
      {test.evidence && (
        <div className="mt-2.5">
          ...
          >
            {showEvidence ? t('cancerScreen.clinicalNoteHide', "Hide clinical note ▲") : t('cancerScreen.clinicalNoteShow', "Clinical note ▼")}
          </button>
          {showEvidence && (
            <p className="text-xs text-[#6B5F4A] dark:text-amber-350 bg-[#F3EAD8]/60 dark:bg-amber-950/20 border border-[#E7DECD] dark:border-amber-900/35 rounded-xl p-3 mt-2 leading-relaxed">
              {t(test.evidenceKey, test.evidence)}
            </p>
          )}
        </div>
      )}

  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-6 sub-page-internal-header">
        ...
        <div>
          <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase">{t('habits.fasting', 'Repair · Fasting')}</span>
          <h2 className="text-2xl font-sans font-bold text-slate-800 dark:text-slate-100 leading-none mt-1">{t('dashboard.fastingWindow', 'Give cells a break')}</h2>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-emerald-100 dark:border-emerald-950/20 shadow-[0_8px_30px_rgba(16,185,129,0.04)] rounded-2xl p-6 mb-4 text-center relative overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-1.5 text-xs text-slate-500 dark:text-slate-450 font-bold">
          ...
          <span>{t('dashboard.activeFasting', 'Fasting — repair pathways are working')}</span>
        </div>
      </div>

      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-3xl p-5 mb-4">
        <span className="text-[10px] font-bold text-slate-400 tracking-widest uppercase block mb-4">{t('habits.logHabit', { habit: t('habits.fasting') })}</span>
        
        <div className="flex flex-col gap-4 mb-5">
          <div>
            <label className="text-[10px] font-bold text-slate-500 dark:text-slate-450 mb-1.5 block uppercase tracking-wider">{t('habits.started', 'Started')}</label>
            <input 
              type="datetime-local" 
              value={started}
          ...
          <div>
            <label className="text-[10px] font-bold text-slate-500 dark:text-slate-450 mb-1.5 block uppercase tracking-wider">{t('habits.ended', 'Ended')}</label>
            <input 
              type="datetime-local" 
              value={ended}
      ...
      <div>
        <span className="text-[10px] font-bold text-slate-400 tracking-widest uppercase block mb-3">{t('fasting.recentFasts', 'Recent Fasts')}</span>
        {loadingHistory ? (
          ...
          <div className="text-center py-8">
            <p className="text-xs text-slate-400">{t('common.noDataFound')}</p>
          </div>
        ) : (

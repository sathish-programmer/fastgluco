...
export const EnvironmentalExposuresLogScreen: React.FC<EnvironmentalExposuresLogScreenProps> = ({ onBack, onBookAppointment, onNavigateToShop }) => {
  ...
  const getStatusBadge = (score: number | null, countFilled: boolean) => {
    if (!countFilled) return <span className="text-[10px] bg-slate-100 text-slate-400 font-bold px-2 py-0.5 rounded-full">{t('environmental.notLogged', 'Not Logged')}</span>;
    if (score === 0) return <span className="text-[10px] bg-emerald-50 text-emerald-600 font-bold px-2 py-0.5 rounded-full">{t('environmental.safe', 'Safe')}</span>;
    return <span className="text-[10px] bg-rose-50 text-rose-600 font-bold px-2 py-0.5 rounded-full">{t('environmental.risk', 'Risk')} ({score})</span>;
  };
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4 mb-8 sub-page-internal-header px-1">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase">{t('environmental.damageBreadcrumb', 'Damage · Environment')}</span>
            <h2 className="text-2xl font-sans font-bold text-slate-800 dark:text-slate-50 leading-none mt-1">
              {currentView === 'hub' && t('environmental.environment', 'Environment')}
              {currentView === 'air' && t('environmental.airPollution', 'Air Pollution')}
              {currentView === 'water' && t('environmental.waterCarcinogens', 'Water Carcinogens')}
              {currentView === 'pesticides' && t('environmental.pesticideExposure', 'Pesticide Exposure')}
              {currentView === 'microplastics' && t('environmental.microplasticsExposure', 'Microplastics Exposure')}
            </h2>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {hasAnyAnswer && (
            <button
              ...
              title={t('environmental.resetToday', 'Reset Today')}
            >
              <RotateCcw className="w-3.5 h-3.5" />
              <span className="hidden xs:inline">{t('environmental.resetToday', 'Reset Today')}</span>
            </button>
          )}
          <span className={`text-[10px] font-extrabold uppercase px-2 py-1 rounded-lg border ${todayLogId || hasAnyAnswer ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border-emerald-200/60 dark:border-emerald-800/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border-slate-200/60 dark:border-slate-700/60'}`}>
            {todayLogId || hasAnyAnswer ? t('environmental.loggedToday', 'Logged (Today)') : t('environmental.dailyCheckin', 'Daily Check-in')}
          </span>
        </div>
      </div>
        ...
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col sm:flex-row items-center gap-6 sm:gap-8">
            <div className="relative w-28 h-28 flex items-center justify-center shrink-0">
              ...
              <div className="absolute flex flex-col items-center">
                ...
                <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">{t('environmental.score', 'Score')}</span>
              </div>
            </div>

            <div className="flex-1 text-center sm:text-left space-y-2">
              <h3 className="font-extrabold text-slate-850 dark:text-slate-100 text-sm">{t('environmental.exposureScore', 'Environmental Exposure Score')}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                {t('environmental.exposureScoreDesc', 'Your score ranges from 0 (Safe) to -4 (High Exposure). Minimize daily environmental carcinogen exposure to reduce systemic DNA damage.')}
              </p>
              {hasAnyAnswer && (
                <div className="pt-2">
                  {overallScore === 0 && (
                    <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs p-4 rounded-2xl font-bold flex items-center gap-2.5">
                      ...
                      {t('environmental.safeFeedback', 'Excellent! You have very low exposure risks today. Keep following these best practices.')}
                    </div>
                  )}
                  {overallScore < 0 && overallScore >= -2 && (
                    <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 text-amber-700 dark:text-amber-400 text-xs p-4 rounded-2xl font-bold flex items-center gap-2.5">
                      ...
                      {t('environmental.moderateFeedback', 'Moderate risk flagged. Take simple steps like carbon-filtering water and avoiding peak outdoor hours.')}
                    </div>
                  )}
                  {overallScore <= -3 && (
                    <div className="bg-rose-50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/30 text-rose-700 dark:text-rose-400 text-xs p-4 rounded-2xl font-bold">
                      <div className="flex items-center gap-2">
                        <ShieldAlert className="h-4 w-4 shrink-0" />
                        {t('environmental.highFeedback', 'High environmental risk. We suggest scheduling a preventative oncology checkup.')}
                      </div>
                      {onBookAppointment && (
                        <button 
                        ...
                        >
                          {t('environmental.bookConsultation', 'Book Preventive Oncologist Consultation')}
                        </button>
                      )}
                    </div>
                  )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            >
              <div className="space-y-1.5">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block flex items-center gap-1.5">
                  01 · {t('environmental.outdoorAirTitle', 'Outdoor Air, Smoke & Fumes')}
                  <span className="text-[9px] bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-md font-extrabold">{t('environmental.aqiTracked', 'AQI Tracked')}</span>
                </span>
                <span className="font-bold text-slate-800 dark:text-slate-200 text-base block group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{t('environmental.airPassiveSmoke', 'Air Pollution & Passive Smoke')}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block">{t('environmental.airPassiveDesc', 'Live AQI widget, second-hand smoke & smog test')}</span>
              </div>
            ...
            >
              <div className="space-y-1.5">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block flex items-center gap-1.5">
                  02 · {t('environmental.waterContaminants', 'Water Contaminants')}
                  <span className="text-[9px] bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-md font-extrabold">{t('environmental.carbonRo', 'Carbon + RO')}</span>
                </span>
                <span className="font-bold text-slate-800 dark:text-slate-200 text-base block group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{t('env.waterPollution', 'Water Pollution')}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block">{t('environmental.dualFiltration', 'Dual Filtration: Activated Carbon + RO')}</span>
              </div>
            ...
            >
              <div className="space-y-1.5">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">03 · {t('environmental.produceFarming', 'Produce & Farming')}</span>
                <span className="font-bold text-slate-800 dark:text-slate-200 text-base block group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{t('env.pesticidesExposure', 'Pesticides Exposure')}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block">{t('environmental.produceRisks', 'Non-organic & produce chemical risks')}</span>
              </div>
            ...
            >
              <div className="space-y-1.5">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">04 · {t('environmental.plasticContainers', 'Plastic Containers')}</span>
                <span className="font-bold text-slate-800 dark:text-slate-200 text-base block group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{t('env.microplasticsExposure', 'Microplastics Exposure')}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium block">{t('environmental.heatedContainers', 'Heated food containers & bottled water')}</span>
              </div>
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800 mt-6">
            ...
            >
              {loading ? t('common.saving') : t('environmental.saveExposureLog', 'Save Exposure Log')}
            </button>
            {!hasAnyAnswer && (
              <p className="text-[10px] text-center text-slate-450 mt-2.5 font-bold uppercase tracking-wider">{t('env.answerAtLeastOne', 'Please answer at least one category to save.')}</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="space-y-6 animate-in slide-in-from-right duration-250">
          ...
          <div className="space-y-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-sm">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-4">
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">{t('env.personalizedRiskQ', 'Personalized Risk Questionnaire')}</span>
              <h3 className="text-base font-extrabold text-slate-900 dark:text-slate-100 mt-0.5">{t('env.airAssessmentTitle', 'Air & Passive Smoke Exposure Assessment')}</h3>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-slate-850 dark:text-slate-100 leading-relaxed mb-3.5">
                  {t('env.airQ1', 'Does your work or daily routine require you to stay outdoors for more than 2 hours on days with poor air quality (AQI > 150)?')}
                </p>
              ...
              <p className="text-sm font-semibold text-slate-850 dark:text-slate-100 leading-relaxed mb-3">
                {t('env.airQ2', 'Do you use mosquito coils or burn incense (agarbatti) in closed, unventilated indoor spaces daily?')}
              </p>
              <div className="flex gap-3">
                <button 
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
              ...
              <p className="text-sm font-semibold text-slate-850 dark:text-slate-100 leading-relaxed mb-3">
                {t('env.airQ3', 'Are you regularly exposed to passive second-hand cigarette/bidi smoke at home or in your workplace?')}
              </p>
              <div className="flex gap-3">
                <button 
            ...
            {/* Recommendation & Doctor Trigger if Air Score is severe */}
            {(airQ1 === true || airQ2 === true || airQ3 === true) && (
              <ConsultationBanner
            ...
            >
              {t('env.doneWithAirCategory', 'Done with Air Exposure Category')}
            </button>
          </div>
        </div>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm font-semibold text-slate-850 dark:text-slate-100 leading-relaxed mb-4">
                {t('env.waterQ1', 'Do you know that your regular drinking water is free of carcinogenic contaminants (e.g., heavy metals, pesticides, PFAS)?')}
              </p>
            ...
            <div className="pt-2">
              ...
              >
                ...
                <span>{t('waterCarcinogenFaq')}</span>
              </button>
            </div>
      ...
      {currentView === 'pesticides' && (
        <div className="space-y-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-sm animate-in slide-in-from-right duration-250">
          <div>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-2">{t('env.questionNum', 'Question')} 1</p>
            <p className="text-sm font-semibold text-slate-850 dark:text-slate-100 leading-relaxed mb-4">
              {t('env.pesticidesQ1', 'Do you regularly consume conventionally grown produce without taking steps to reduce pesticide residues (e.g., washing thoroughly or choosing lower-residue options when possible)?')}
            </p>
            ...
            <div className="bg-amber-50/50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 rounded-2xl p-5 space-y-3">
              ...
              <ul className="text-xs text-amber-700 dark:text-amber-450 space-y-2 list-disc pl-4 leading-relaxed font-semibold">
                ...
                <li>{t('peelSkinsAdvice')}</li>
                <li>{t('dirtyDozenAdvice')}</li>
              </ul>
            </div>
      ...
      {currentView === 'microplastics' && (
        <div className="space-y-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-sm animate-in slide-in-from-right duration-250">
          <div>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mb-2">{t('env.questionNum', 'Question')} 1</p>
            <p className="text-sm font-semibold text-slate-850 dark:text-slate-100 leading-relaxed mb-4">
              {t('env.microplasticsQ1', 'Do you regularly drink from plastic water bottles or consume food stored, served, or heated in plastic containers (especially non-food-grade plastics)?')}
            </p>
            <div className="flex gap-3 mb-6">
              <button 
        ...
        <div className="space-y-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-sm animate-in slide-in-from-right duration-250">
          ...
          <div className="bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 rounded-2xl p-5 space-y-3">
            ...
            <ul className="text-xs text-emerald-700 dark:text-emerald-450 space-y-2 list-disc pl-4 font-semibold leading-relaxed">
              ...
              <li><strong>{t('nonPlasticWaterStorage')}</strong>: {t('nonPlasticWaterDesc')}</li>
              <li><strong>{t('glassStainlessJars')}</strong>: {t('glassStainlessDesc')}</li>
            </ul>
          </div>

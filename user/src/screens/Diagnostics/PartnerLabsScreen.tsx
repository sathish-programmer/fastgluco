  ...
  return (
    <div className="pb-24 pt-6 px-4 max-w-5xl mx-auto bg-slate-50 min-h-screen font-sans antialiased">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase">{t('diagnosticCenterSelect', 'Select Center')}</span>
      ...
      {loading ? (
        <div className="text-center py-10 animate-pulse text-slate-400 font-bold">{t('findingDiagnosticCenters', 'Finding diagnostic centers...')}</div>
      ) : labs.length === 0 ? (
              ...
              <div key={labTest._id} className="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">{t('common.price', 'Price')}</span>
                ...
                <div className="flex items-center gap-3 mb-5">
                  <div className="flex items-center gap-1 text-[10px] font-bold text-slate-600 bg-slate-100 px-2 py-1 rounded-lg">
                    <Clock className="h-3 w-3" /> {t('reportsIn', 'Reports in')} {labTest.turnaroundTimeHours} hrs
                  </div>
                  {lab.isHomeCollectionAvailable && (
                    <div className="text-[10px] font-bold text-green-700 bg-green-50 border border-green-200 px-2 py-1 rounded-lg">
                      {t('homeCollectionAvailable', 'Home Collection Available')}
                    </div>
                  )}
                </div>
                ...
                {labTest.preparationInstructions && (
                  <div className="bg-orange-50 border border-orange-100 p-3 rounded-xl mb-4">
                    <span className="text-[10px] font-bold text-orange-600 uppercase block mb-1">{t('prepNote', 'Preparation Note')}</span>
                ...
                >
                  {t('selectCenter', 'Select this Center')}
                </button>
              </div>
            );
          })}

  ...
  return (
    <div className="pb-32 pt-6 px-4 max-w-5xl mx-auto bg-slate-50 min-h-screen font-sans antialiased">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <span className="text-[10px] font-bold text-slate-400 tracking-[0.2em] uppercase">{t('diag.step2of3', 'Step 2 of 3')}</span>
          <h2 className="text-2xl font-bold text-slate-800 leading-none mt-1">{t('diag.collectionDetails', 'Collection Details')}</h2>
        </div>
      </div>

      <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm mb-6">
        <h3 className="font-bold text-slate-800 mb-4 text-sm uppercase tracking-wider">{t('diag.howToGiveSample', 'How would you like to give your sample?')}</h3>
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <span className={`block font-bold ${collectionType === 'HOME' ? 'text-indigo-900' : 'text-slate-700'}`}>{t('diag.homeVisit', 'Home Visit')}</span>
            <span className="text-[10px] text-slate-500 mt-1 block">Trained phlebotomist visits your home (₹{homeCollectionFee})</span>
            {!lab.isHomeCollectionAvailable && <span className="text-[10px] font-bold text-red-500 mt-1 block">{t('diag.notAvailableForLab', 'Not available for this lab')}</span>}
          </button>
          ...
          >
            ...
            <span className={`block font-bold ${collectionType === 'LAB_VISIT' ? 'text-indigo-900' : 'text-slate-700'}`}>{t('diag.visitCenter', 'Visit Center')}</span>
            <span className="text-[10px] text-slate-500 mt-1 block">{t('diag.visitCenterFree', 'You walk into the diagnostic center (Free)')}</span>
          </button>
        </div>
      </div>
      ...
      {collectionType === 'HOME' && (
        <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm mb-6">
          <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2">
            <MapPin className="h-4 w-4 text-slate-400" /> {t('diagnostics.collectionAddress', 'Collection Address')}
          </h3>
          <textarea 
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm mb-6">
        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2">
          <CalendarIcon className="h-4 w-4 text-slate-400" /> {t('diagnostics.chooseDate', 'Choose a Date')}
        </h3>
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm mb-6">
        <h3 className="font-bold text-slate-800 mb-4 text-sm flex items-center gap-2">
          <Clock className="h-4 w-4 text-slate-400" /> {t('diagnostics.chooseTimeSlot', 'Choose a Time Slot')}
        </h3>
        
        {(lab.holidays || []).includes(date) ? (
          <div className="py-8 text-center bg-rose-50 rounded-2xl border border-rose-100">
            <span className="text-3xl block mb-2">⛱️</span>
            <h4 className="font-bold text-rose-800 text-lg">{t('diag.labClosed', 'Lab Closed')}</h4>
            <p className="text-rose-600 text-sm mt-1">{t('diag.labHoliday', 'This laboratory is on holiday for the selected date.')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="col-span-3 py-6 text-center text-slate-500 bg-slate-50 rounded-xl border border-slate-100 border-dashed">
              {t('diagnostics.noTimeSlotsLab', 'No time slots available for this lab.')}
            </div>
          )}
        </div>
        )}
      ...
      <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm mb-6">
        <h3 className="font-bold text-slate-800 mb-3 text-sm">{t('diag.specialInstructions', 'Special Instructions (Optional)')}</h3>
        <input 
          type="text" 
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] max-w-5xl mx-auto z-40">
        <div className="flex items-center justify-between gap-4">
          <div>
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">{t('diag.totalAmount', 'Total Amount')}</span>

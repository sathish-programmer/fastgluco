...
export const AiBannerQuickNudge: React.FC<AiBannerQuickNudgeProps> = ({
  ...
  const displayTitle = title || t('habits.aiBannerTitle', 'Log all daily habits 10x faster');
  const displaySubtitle = subtitle || t('habits.aiBannerSubtitle', 'Log habits & upload reports in under 60s via AI Assistant');

  return (
    <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 dark:from-blue-700 dark:via-indigo-700 dark:to-violet-800 text-white rounded-2xl sm:rounded-3xl p-3.5 sm:p-4 shadow-sm hover:shadow-md transition-all duration-300 border border-white/20 group">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1 text-[9px] font-black uppercase bg-gradient-to-r from-amber-400 to-yellow-300 text-slate-950 px-2 py-0.5 rounded-full shadow-xs tracking-wider">
                <Sparkles className="h-2.5 w-2.5 fill-slate-950" /> {t('habits.aiPowered', 'AI POWERED')}
              </span>
            </div>
